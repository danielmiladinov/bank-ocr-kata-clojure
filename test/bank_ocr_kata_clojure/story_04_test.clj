...
(deftest to-glyphs
  (testing "single digit ints"
    (is (= p/zero (p/to-glyphs 0)))
    (is (= p/one (p/to-glyphs 1)))
    (is (= p/two (p/to-glyphs 2)))
    (is (= p/three (p/to-glyphs 3)))
    (is (= p/four (p/to-glyphs 4)))
    (is (= p/five (p/to-glyphs 5)))
    (is (= p/six (p/to-glyphs 6)))
    (is (= p/seven (p/to-glyphs 7)))
    (is (= p/eight (p/to-glyphs 8)))
    (is (= p/nine (p/to-glyphs 9))))
...
(deftest replace-in-glyphs
  (testing "position 0"
    (let [input    (p/to-glyphs "123456789")
          ...
          actual   (p/replace-in-glyphs input 0 p/zero)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "223456789")
          actual   (p/replace-in-glyphs input 0 p/two)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "323456789")
          actual   (p/replace-in-glyphs input 0 p/three)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "423456789")
          actual   (p/replace-in-glyphs input 0 p/four)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "523456789")
          actual   (p/replace-in-glyphs input 0 p/five)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "623456789")
          actual   (p/replace-in-glyphs input 0 p/six)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "723456789")
          actual   (p/replace-in-glyphs input 0 p/seven)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "823456789")
          actual   (p/replace-in-glyphs input 0 p/eight)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "923456789")
          actual   (p/replace-in-glyphs input 0 p/nine)]
      (is (= expected actual))))
  (testing "position 1"
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "103456789")
          actual   (p/replace-in-glyphs input 1 p/zero)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "113456789")
          actual   (p/replace-in-glyphs input 1 p/one)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "133456789")
          actual   (p/replace-in-glyphs input 1 p/three)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "143456789")
          actual   (p/replace-in-glyphs input 1 p/four)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "153456789")
          actual   (p/replace-in-glyphs input 1 p/five)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "163456789")
          actual   (p/replace-in-glyphs input 1 p/six)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "173456789")
          actual   (p/replace-in-glyphs input 1 p/seven)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "183456789")
          actual   (p/replace-in-glyphs input 1 p/eight)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "193456789")
          actual   (p/replace-in-glyphs input 1 p/nine)]
      (is (= expected actual))))
  (testing "position 2"
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "120456789")
          actual   (p/replace-in-glyphs input 2 p/zero)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "121456789")
          actual   (p/replace-in-glyphs input 2 p/one)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "122456789")
          actual   (p/replace-in-glyphs input 2 p/two)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "124456789")
          actual   (p/replace-in-glyphs input 2 p/four)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "125456789")
          actual   (p/replace-in-glyphs input 2 p/five)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "126456789")
          actual   (p/replace-in-glyphs input 2 p/six)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "127456789")
          actual   (p/replace-in-glyphs input 2 p/seven)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "128456789")
          actual   (p/replace-in-glyphs input 2 p/eight)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "129456789")
          actual   (p/replace-in-glyphs input 2 p/nine)]
      (is (= expected actual))))
  (testing "position 3"
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123056789")
          actual   (p/replace-in-glyphs input 3 p/zero)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123156789")
          actual   (p/replace-in-glyphs input 3 p/one)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123256789")
          actual   (p/replace-in-glyphs input 3 p/two)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123356789")
          actual   (p/replace-in-glyphs input 3 p/three)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123556789")
          actual   (p/replace-in-glyphs input 3 p/five)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123656789")
          actual   (p/replace-in-glyphs input 3 p/six)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123756789")
          actual   (p/replace-in-glyphs input 3 p/seven)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123856789")
          actual   (p/replace-in-glyphs input 3 p/eight)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123956789")
          actual   (p/replace-in-glyphs input 3 p/nine)]
      (is (= expected actual))))
  (testing "position 4"
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123406789")
          actual   (p/replace-in-glyphs input 4 p/zero)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123416789")
          actual   (p/replace-in-glyphs input 4 p/one)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123426789")
          actual   (p/replace-in-glyphs input 4 p/two)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123436789")
          actual   (p/replace-in-glyphs input 4 p/three)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123446789")
          actual   (p/replace-in-glyphs input 4 p/four)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123466789")
          actual   (p/replace-in-glyphs input 4 p/six)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123476789")
          actual   (p/replace-in-glyphs input 4 p/seven)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123486789")
          actual   (p/replace-in-glyphs input 4 p/eight)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123496789")
          actual   (p/replace-in-glyphs input 4 p/nine)]
      (is (= expected actual))))
  (testing "position 5"
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123450789")
          actual   (p/replace-in-glyphs input 5 p/zero)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123451789")
          actual   (p/replace-in-glyphs input 5 p/one)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123452789")
          actual   (p/replace-in-glyphs input 5 p/two)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123453789")
          actual   (p/replace-in-glyphs input 5 p/three)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123454789")
          actual   (p/replace-in-glyphs input 5 p/four)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123455789")
          actual   (p/replace-in-glyphs input 5 p/five)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123457789")
          actual   (p/replace-in-glyphs input 5 p/seven)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123458789")
          actual   (p/replace-in-glyphs input 5 p/eight)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123459789")
          actual   (p/replace-in-glyphs input 5 p/nine)]
      (is (= expected actual))))
  (testing "position 6"
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456089")
          actual   (p/replace-in-glyphs input 6 p/zero)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456189")
          actual   (p/replace-in-glyphs input 6 p/one)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456289")
          actual   (p/replace-in-glyphs input 6 p/two)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456389")
          actual   (p/replace-in-glyphs input 6 p/three)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456489")
          actual   (p/replace-in-glyphs input 6 p/four)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456589")
          actual   (p/replace-in-glyphs input 6 p/five)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456689")
          actual   (p/replace-in-glyphs input 6 p/six)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456889")
          actual   (p/replace-in-glyphs input 6 p/eight)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456989")
          actual   (p/replace-in-glyphs input 6 p/nine)]
      (is (= expected actual))))
  (testing "position 7"
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456709")
          actual   (p/replace-in-glyphs input 7 p/zero)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456719")
          actual   (p/replace-in-glyphs input 7 p/one)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456729")
          actual   (p/replace-in-glyphs input 7 p/two)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456739")
          actual   (p/replace-in-glyphs input 7 p/three)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456749")
          actual   (p/replace-in-glyphs input 7 p/four)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456759")
          actual   (p/replace-in-glyphs input 7 p/five)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456769")
          actual   (p/replace-in-glyphs input 7 p/six)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456779")
          actual   (p/replace-in-glyphs input 7 p/seven)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456799")
          actual   (p/replace-in-glyphs input 7 p/nine)]
      (is (= expected actual))))
  (testing "position 8"
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456780")
          actual   (p/replace-in-glyphs input 8 p/zero)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456781")
          actual   (p/replace-in-glyphs input 8 p/one)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456782")
          actual   (p/replace-in-glyphs input 8 p/two)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456783")
          actual   (p/replace-in-glyphs input 8 p/three)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456784")
          actual   (p/replace-in-glyphs input 8 p/four)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456785")
          actual   (p/replace-in-glyphs input 8 p/five)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456786")
          actual   (p/replace-in-glyphs input 8 p/six)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456787")
          actual   (p/replace-in-glyphs input 8 p/seven)]
      (is (= expected actual)))
    (let [input    (p/to-glyphs "123456789")
          expected (p/to-glyphs "123456788")
          actual   (p/replace-in-glyphs input 8 p/eight)]
      (is (= expected actual)))))
